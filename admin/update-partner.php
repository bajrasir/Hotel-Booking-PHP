	...
	if(isset($_GET['id']))
	 {
		...
		$sql="SELECT * FROM tbl_partners where id=$id";
		...
		else
		{
			...
			$_SESSION['no-category-found'] ="<div class='error>Partners Not Found.</div>";
				...
				<table class="tbl-30">
							...
							if ($current_image!="")
							 {
								// display the image
								?>
								<img src="<?php echo SITEURL; ?>images/admin/partners/<?php echo $current_image; ?>" width='100px';>
								<?php
					...
					<tr>
						<td>
							...
							<input type="submit" name="submit" value="Update Partners" class="btn-secondary">
						</td>
					</tr>
		...
		if (isset($_FILES['image']['name']))
		 {
			...
			if($image_name!="")
			{
		 	...
		 	// get the extenstion of our image(jpg,png,gif) eg: food1.jpg
			 	...
			 	$image_name = "Partner_Category_".rand(000,999).'.'.$ext; //eg:Food_Category_834.jpg
		 	...
		 	if ($current_image!="")
		 	{
		 	 	$remove_path="../images/admin/partners/".$current_image;
		...
		if ($res2==true)
		 {
		 	...
		 	// category updated
			$_SESSION['update']="<div class='success'>Partner Successfully Updated.</div>";
			header('location:'.SITEURL.'admin/manage-partner.php');
		}
		else
		{
			// failed to update category
			$_SESSION['update']="<div class='error'>Partner Updated Failed.</div>";
			header('location:'.SITEURL.'admin/manage-partner.php');
		}
